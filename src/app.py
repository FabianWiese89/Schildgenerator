...
class QRCodeGeneratorApp:
    ...
    def on_single_generate(self):
        ...
        if choice == 4:
            self.generate_pdf_einzeln(lagerplatz_vis, qr_data, output, 4)
        else:
            self.generate_pdf_einzeln(lagerplatz_vis, qr_data, output, 5)
        self.single_status.config(text="Fertig")
        messagebox.showinfo("Fertig", f"PDF erfolgreich erstellt:\n{output}")

    def generate_pdf_einzeln(self, text_vis, qr_data, output, zeilen):
        if zeilen == 4:
